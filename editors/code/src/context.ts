...
export class Context {
  ...
  static create(
    ...
    if (!commandExistsSync(configuration.serverPath)) {
      return new Error(
        ...
        'the isle-analyzer Visual Studio Code extension README to install the language ' +
        'server.',
      );
    }
  ...
  configureLanguage(): void {
    const disposable = vscode.languages.setLanguageConfiguration('yuyang', {
      onEnterRules: [
        {
   ...
   *
   * The "client" is an object that sends messages to the language server, which in ISLE's case is
   * the `isle-analyzer` executable. Unlike registered extension commands such as
   * `isle-analyzer.serverVersion`, which are manually executed by a VS Code user via the command
  ...
  async startClient(): Promise<void> {
    ...
    // that is 'ISLE Language Server'). For more information, see:
    ...
    const traceOutputChannel = vscode.window.createOutputChannel(
      'ISLE Analyzer Language Server Trace',
    );
    const clientOptions: lc.LanguageClientOptions = {
      documentSelector: [{ scheme: 'file', language: 'isle' }],
      traceOutputChannel,
    };

    const client = new lc.LanguageClient(
      'isle-analyzer',
      'ISLE Language Server',
      serverOptions,
      clientOptions,
    );
    ...
    this.extensionContext.subscriptions.push(disposable);
    this.client = client;
    // Wait for the ISLE Language Server initialization to complete,
